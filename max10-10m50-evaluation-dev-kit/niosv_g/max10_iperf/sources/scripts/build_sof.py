    ...
    with open (file,'r') as f1:
        ...
        for i,line in enumerate(line_f1):
            if 'project_exists ' in line:
                ...
                qsys_name = line_f1[i].replace(" ","").replace("{","").replace("}","").split("create_system",1)[1]
                print(qsys_name)
...
generate_clkctrl = subprocess.Popen([
    "qsys-generate",
    "clkctrl.qsys",
    "--synthesis=VERILOG"
], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=cwd_1)

for line in generate_clkctrl.stdout:
    logfile.write(line)
generate_clkctrl.wait()


generate_q_sys = subprocess.Popen([
    "qsys-generate",
    "q_sys.qsys",
    "--synthesis=VERILOG"
], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=cwd_1)

for line in generate_q_sys.stdout:
    logfile.write(line)
generate_q_sys.wait()
...
# Generate ip
# generate_ip = subprocess.Popen(["qsys_generate", "--synthesis=verilog","{}".format(qsys_dir)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT,cwd=cwd_1)
# for line in generate_ip.stdout:
#     logfile.write(line)
# generate_ip.wait()
